...
async function populateDropDown() {
  try {
    ...
    fetchHistoricalExchangeRates().then((data) => {
...
async function updateDisplay() {
  ...
  displayElement.textContent = `${amount} ${fromCurrency} is ${convertedAmount.toFixed(
    2,
  )} ${toCurrency}`;
}
...
async function fetchExchangeRates(fromCurrency, toCurrency) {
  try {
    const response = await fetch(
      `${exchangeRatesApi}latest/currencies/${fromCurrency}/${toCurrency}.json`,
    );
...
async function fetchHistoricalExchangeRates() {
  ...
  while (currentDateIterator <= new Date(endDate)) {
    ...
    try {
      ...
      console.error(
        `Error fetching historical exchange rates for ${currentDateFormatted}: `,
        error,
      );
    }
...
function drawExchangeRateChart(historicalData) {
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: `from ${fromCurrency} to ${toCurrency}`,
          data: exchangeRates,
